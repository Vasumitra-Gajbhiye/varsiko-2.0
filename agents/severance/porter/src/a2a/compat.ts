/**
 * Nasiko speaks A2A 1.0 method names; we accept both 1.0 and 0.3 forms.
 */
...
/** Rewrite inbound JSON-RPC body in place for A2A 1.0 → 0.3 compatibility. */
...
export function extractNasikoToken(headers: Record<string, string | string[] | undefined>): string | undefined {
  const raw =
    headers['x-nasiko-agent-token'] ??
    headers['X-Nasiko-Agent-Token'] ??
    headers['x-nasiko-agent-token'.toLowerCase()];
...
export function makeTask(opts: {
  ...
  const artifacts = (opts.artifacts ?? []).map((a) => {
    ...
    if (a.data !== undefined) {
      ...
      parts.push({ kind: 'text', text: typeof a.text === 'string' ? a.text : JSON.stringify(a.data) });
