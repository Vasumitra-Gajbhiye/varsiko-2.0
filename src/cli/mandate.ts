...
Issues a signed, single-use mandate authorising ONE server and ONE migration.
  ...
  --ttl-minutes <n>       Time to START spending. Default 10
...
export async function mandateCommand(
  ...
  const { values: v } = parseArgs({
    ...
    options: {
      ...
      'ttl-minutes': { type: 'string', default: '10' },
  ...
  const serverType = need('server-type', v['server-type']);
  const location = need('location', v.location);
  ...
  const maxMonthly = positive('--max-monthly', v['max-monthly'], 10_000);
  const ttlMinutes = positive('--ttl-minutes', v['ttl-minutes'], 240);
  ...
  const pinned = PINNED_PRICES_USD_MONTH[serverType];
  if (pinned === undefined) {
  ...
  const mandate: Mandate = {
    ...
    scope: v['no-dns'] ? SCOPES.filter((s) => !DNS_SCOPES.includes(s)) : SCOPES,
  ...
  io.out(`  cloud-init template    sha256 ${mandate.provision.cloud_init_sha256.slice(0, 16)}...`);
